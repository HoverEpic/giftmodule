...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create')))
{
	...
	/*
	// Ref bis
	$morehtmlref.=$form->editfieldkey("RefBis", 'ref_client', $object->ref_client, $object, $user->rights->giftmodule->creer, 'string', '', 0, 1);
	$morehtmlref.=$form->editfieldval("RefBis", 'ref_client', $object->ref_client, $object, $user->rights->giftmodule->creer, 'string', '', null, null, '', 1);
	// Thirdparty
	...
	if (! empty($conf->projet->enabled))
	{
	    ...
	    if ($user->rights->giftmodule->creer)
	    {
	        if ($action != 'classify')
	        {
	...
	*/
	...
	if ($action != 'presend')
	{
	    ...
	    $filedir = $conf->giftmodule->dir_output . '/' . $objref;
	    $urlsource = $_SERVER["PHP_SELF"] . "?id=" . $object->id;
	    $genallowed = $user->rights->giftmodule->read;	// If you can read, you can build the PDF to read content
	    $delallowed = $user->rights->giftmodule->create;	// If you can create/edit, you can remove a file on card
	    print $formfile->showdocuments('giftmodule', $objref, $filedir, $urlsource, $genallowed, $delallowed, $object->modelpdf, 1, 0, 0, 28, 0, '', '', '', $soc->default_lang);
		*/

	    // Show links to link elements
	    $linktoelem = $form->showLinkToObjectBlock($object, null, array('gift'));
	    ...
	    $morehtmlright = '<a href="'.dol_buildpath('/giftmodule/gift_info.php', 1).'?id='.$object->id.'">';
	    ...
	    $somethingshown = $formactions->showactions($object, 'gift', $socid, 1, '', $MAXEVENT, '', $morehtmlright);
