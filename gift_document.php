...
/**
 *  \file       gift_document.php
 *  \ingroup    giftmodule
 *  \brief      Tab for documents linked to Gift
 */
...
$langs->loadLangs(array("giftmodule@giftmodule","companies","other"));
...
$hookmanager->initHooks(array('giftdocument'));     // Note that conf->hooks_modules contains array
...
$title=$langs->trans("Gift").' - '.$langs->trans("Files");
$help_url='';
...
if ($object->id)
{
	...
	dol_fiche_head($head, 'document', $langs->trans("Gift"), -1, 'gift@giftmodule');
	...
	$modulepart = 'giftmodule';
	//$permission = $user->rights->giftmodule->create;
	$permission = 1;
	//$permtoedit = $user->rights->giftmodule->create;
	$permtoedit = 1;
	$param = '&id=' . $object->id;

	//$relativepathwithnofile='gift/' . dol_sanitizeFileName($object->id).'/';
	$relativepathwithnofile='gift/' . dol_sanitizeFileName($object->ref).'/';
